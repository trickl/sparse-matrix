...
public class GramSchmidtCoefficients {
   ...
   public GramSchmidtCoefficients(DoubleMatrix2D A, DoubleMatrix2D V, DoubleMatrix2D T, int maxRank) {
      ...
      this.V = V;
      this.T = T;
      this.aCoefficients = new DenseDoubleMatrix2D(A.rows(), A.rows());
      this.vCoefficients = new DenseDoubleMatrix2D(A.rows(), V.rows());      
      this.sMaxRank = maxRank;
      orthonormalize();
   }
      ...
      if (sRank < A.rows() || sRank > sMaxRank) {
         ...
         for (int aRow = 0, hRow = 0; aRow < sRank; ++aRow) {
            if (orthogonalNorms.get(aRow) > tolerance) {
               ...
               aCoefficientsCopy.viewRow(hRow).assign(aCoefficients.viewRow(aRow), Functions.plusMult(1 / Math.sqrt(norm2)));
               vCoefficientsCopy.viewRow(hRow).assign(vCoefficients.viewRow(aRow), Functions.plusMult(1 / Math.sqrt(norm2)));
